...
int sonElDamYunCpu(char* bufferId1,char*bufferId2,t_log*logger){
	if(string_equals_ignore_case(bufferId1,"DAM") && string_equals_ignore_case(bufferId2,"CPU"))
		log_info(logger,"la conexion 1 es diego");
		return 1;
	if(string_equals_ignore_case(bufferId2,"DAM") && string_equals_ignore_case(bufferId1,"CPU"))
